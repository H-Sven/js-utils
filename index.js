/*
 * @Author: Siwen
 * @Date: 2020-06-15 09:54:48
 * @LastEditors: Siwen
 * @LastEditTime: 2020-06-15 11:41:57
 * @Description: 常用utils库
 */
let environmentInfo = {
  inited: false,
  /** IE内核 */
...
export default {
   ...
   * @param {number} state 指定符号前:0还是后:1 (默认1)
   * @returns {string}
   */
  getCaption(str, symbol, state = 1) {
   ...
   * @param {string | number} delay 防抖时间
   * @returns {Function}
   */
  debounce(func, delay) {
    let timer
   ...
   * 验证邮箱
   * @method isEmail
   * @param {string | number} email 邮箱地址
   * @returns {Boolean}
   */
  isEmail(email){
   ...
   */
  formatDate(x, y = 'yyyy-MM-dd hh:mm:ss'){
  ...
  downloadCSV(url, params = {}) {
    const finalUrl = `${url}?${this.encodeParams(params)}`
